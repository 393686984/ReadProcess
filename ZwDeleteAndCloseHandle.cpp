...
NTSTATUS IrpSetInformationFileCompletionRoutine(
...
NTSTATUS FixFilePath(PWCHAR* aFixedPath, PWCHAR aFilePath)
{
	...
	for (;;)
	{
		...
		{
			auto vPathBytes = wcslen(aFilePath) * sizeof(aFilePath[0]) + sizeof(UNICODE_NULL);
		...
		{
			INT vPathBytes = wcslen(aFilePath) * sizeof(aFilePath[0]) + sizeof(cNtPrefix);
			vPath = (LPWSTR)ExAllocatePool(NonPagedPool, vPathBytes);
			if (NULL == vPath)
			{
				vStatus = STATUS_INSUFFICIENT_RESOURCES;
...
PMY_SYSTEM_HANDLE_INFORMATION EnumerateHandles()
{
	...
	int status = ZwQuerySystemInformation((SYSTEM_INFORMATION_CLASS)SystemHandleInformation,
		...
		&shiTableSize);
	//TODO: check status		
	shiTable = (PMY_SYSTEM_HANDLE_INFORMATION)ExAllocatePoolWithTag(NonPagedPool, shiTableSize, 0xdeadbeef);
...
VOID R0_CloseAllHandles(PWCHAR FilePath)
{
	MY_SYSTEM_HANDLE_INFORMATION *shiTable;
	PEPROCESS eprocess;
	unsigned long i;
	...
	{
		if (IsFile(shiTable->Handles[i].Object))
		{
	...
	PUNICODE_STRING reg_path)
{
	...
	ZwClose(Mykey);
	OBJECT_ATTRIBUTES MyFileobject = { 0 };
